...
class MonsterService(BaseService):
    def generate_monster(self, base_monster, theme, target_cr=None):
        ...
        system = """
        Eres una veterana diseñadora de monstruos D&D 5e (2024).
        Your job is to apply thematic "templates" to existing monsters, modifying their stats and abilities.

        Return ALWAYS a valid JSON with this EXACT structure (English snake_case) for keys and Spanish for values:
        {
            ...
            "actions": [ 
                {"name": "Attack Name", "desc": "Melee weapon attack: +5 to hit, 1d8+3 damage (slashing)."} 
            ],
            "visual": "Brief description of appearance for DM to read aloud."
        }
        
        IMPORTANT ABOUT ACTIONS:
        - Each action MUST have 'name' (string) and 'desc' (string with complete attack description)
        - Include attack bonus, damage dice, and damage type in the 'desc' field
        - Example: "name": "Espada Ardiente", "desc": "+5 al golpe, 2d8+3 de daño (cortante y fuego)"
        """
        ...
        CRITICAL: Return EXACTLY JSON with English keys (snake_case) and spanish values.
        Use: "name", "stats", "alignment", NOT Spanish variants.
        Ensure actions have BOTH 'name' and 'desc' fields with complete descriptions.
        """

        result = self._generate_content(system, prompt)
        print(f"[LOG] Monster generated by Gemini: {result}")  # DEBUG
        return result
